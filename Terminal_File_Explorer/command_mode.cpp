...
int copy_file_to_dir(string src_file_path, string dest_dir_path)
{
    ...
    out << in.rdbuf();//A pointer to the stream buffer object associated with the stream before the call.
...
int copy_cb(const char* src_path, const struct stat* sb, int type_flag) {
