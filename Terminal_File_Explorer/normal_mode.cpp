...
void print_mode()
{
    ...
    switch(current_mode)
    {
        ...
        default:
            ss << "[NORMAL MODE]";
            cout << "\033[0;36m" << ss.str() << "\033[0m" << " ";

#if 0
            if(is_status_pending)
            {
                cout << "\033[0;36m" << status_str << "\033[0m";
            }
#endif
            cout.flush();
            break;
...
pair<int, int> content_list_print(list<dir_content>::const_iterator itr)
{
    ...
    for(; i < pwd_rank - 1; ++i)
    {
        cout << "\033[0;36m" << ss.str().substr((i*w.ws_col), (i+1)*w.ws_col) << "\033[0m";
        ++cursor_r_pos;
        cursor_init();
    }
    cout << "\033[0;36m" << ss.str().substr(i*w.ws_col) << "\033[0m";
    ++cursor_r_pos;
    cursor_init();
        ...
        while(!refresh_dir)
        {
            ...
            switch(ch)
            {
                ...
                case 'q': // CHANGE
                {
                    ...
                    {
                        screen_clear();
                        cout << "\033[0;91m" << "Exit File Explorer? (y/n):" << "\033[0m" << " ";
                        ch = next_input_char_get();
                        switch(ch)
                        {
                            case 'y':
